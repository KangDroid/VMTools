...
int main(int argc, char** argv) {
    ...
    if (global_info.json_path == filesystem::path("/")) {
        ...
        LOG_E("Json path is not defined, therefore using " + string(DEFAULT_SAMPLE_DIR) + "/assets/sample.json for default json path.");
        global_info.json_path = filesystem::path(string(DEFAULT_SAMPLE_DIR) + "/assets/sample.json");
    }
